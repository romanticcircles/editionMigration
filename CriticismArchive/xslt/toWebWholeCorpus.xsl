...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml"
	...
	<!-- to run a teiCorpus as one html output ; manual file save needed -->
	<xsl:template match="tei:teiCorpus">
		...
		<xsl:variable name="authorID" select="substring-after(tei:TEI[1]/tei:text/tei:body/tei:div/tei:head/tei:bibl/tei:author/tei:ref/@target, 'people.html#')"/>
		<xsl:variable name="pubDate">
			<xsl:value-of select="tei:teiHeader/tei:fileDesc/tei:sourceDesc/tei:biblStruct/*/tei:imprint/tei:date/@when"/>
		</xsl:variable>
		<xsl:variable name="uniqueIDs" select="distinct-values(tei:TEI/tei:text//tei:ref/substring-after(@target, 'people.html#'))" />
		...
		<html xml:lang="en" lang="en" id="{$filename}"
			...
			<head>
				...
				<meta property="og:image" content="{$baseURL}/images/CritArcLogo.png"/>
				...
				<xsl:comment>DO NOT EDIT THIS FILE. This HTML document was generated from a TEI Master on 
					...
					and rerun the document using the xslt (toWebWholeCorpus.xsl) to regenerate this file. </xsl:comment>
				<title>
			...
			<body>
				<nav>
					<div class="logo-section">
						<a href="https://romantic-circles.org/" target="_blank">
							<img class="logo" src="../images/RClogo.png" alt="Romantic Circles ogo"
							/>
						</a>
						<p class="masthead">The Criticism Archive, ed. Mary Waters</p>
					</div>
					<ul class="navBar">
						<li>
				...
				<main>
					...
					<p class="tei">
						<a class="tei" href="../teiCorpus/{$filename}.xml">
					...
					<div class="footer">
						<button onclick="copyLink('{$currentLink}')" class="navy-btn">Copy
							Link</button>
						<p class="noindent"><img src="../images/RClogo.png"
								alt="Romantic Circles logo" height="20" width="20"/>&#160;Ed. Mary
							A. Waters, The Criticism Archive, published by Romantic Circles, 2026. <em>Updated 
								<xsl:value-of
								select="format-date(xs:date($htmlPubDate), '[M01]/[D01]/[Y0001]')"/>.</em><br/>
	...
	<xsl:template match="tei:TEI">
		<!-- for running TEICorpus doc as one web page  -->
		<xsl:apply-templates/>
	</xsl:template>
