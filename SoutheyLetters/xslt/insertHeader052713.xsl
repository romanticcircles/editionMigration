	...
	<xsl:template match="tei:TEI">
		<xsl:choose>
			<xsl:when test="element-available('redirect:write')">
				<xsl:for-each select="tei:text/tei:body/tei:div[@type='letter']">
					<xsl:variable name="letterNumber">
						<xsl:value-of select="@n"/>
					</xsl:variable>
					<redirect:write select="concat('letterEEd.26.', $letterNumber, '.xml')">
						<TEI xmlns="http://www.tei-c.org/ns/1.0">
							<teiHeader>
								<fileDesc>
									...
									<publicationStmt>
										<idno type="edition">
											<xsl:value-of select="concat('letterEEd.26.', $letterNumber)"/>
										</idno>
										<idno type="nines">
											<xsl:value-of select="concat('rce', 1271 + $letterNumber)"/> <!-- adding 1122 makes the first rce number 2000, which is what I checked out from the NINES uniqueid list for Part 3 -->
											<!-- adding 1271 makes the first rce number 3000, which is what I checked out from the NINES uniqueid list for Part 4 -->
										</idno>
							...
							<addrLine>fraistat@umd.edu</addrLine>
									...
									<sourceDesc>
										<p>
											<xsl:choose>
												<xsl:when test="contains(ancestor-or-self::tei:div/tei:head/tei:note[@type='headnote'], 'Previously')">
													<xsl:value-of
														select="substring-after(substring-before(ancestor-or-self::tei:div/tei:head/tei:note[@type='headnote'], 'Previously'), 'MS: ')"/>
													<xsl:text>.  Previously </xsl:text>
													<xsl:value-of
														select="substring-after(ancestor-or-self::tei:div/tei:head/tei:note[@type='headnote'], 'Previously')"
													/>
												</xsl:when>
												<xsl:otherwise>
													<xsl:value-of
														select="substring-after(substring-before(ancestor-or-self::tei:div/tei:head/tei:note[@type='headnote'], 'Unpublished'), 'MS: ')"/>
													<xsl:text>.  Not previously published.</xsl:text>
												</xsl:otherwise>
											</xsl:choose>
										</p>
								...
								<profileDesc>
									<textClass>
										<catRef target="#g7 #g27" scheme="#genre"/>
										<catRef target="./people.html" scheme="#people"/>
										<catRef target="./places.html" scheme="#places"/>
									</textClass>
								</profileDesc>
								<revisionDesc>
									<change n="8" when="2013-05-27" who="#LM">
									...
									<change n="7" when="2013-04-22" who="#LM">
										<label>Changed by</label>
										<name>Laura Mandell</name>
										<list>
									...
									<change who="#LM" when="2013-02-01" n="6">
										<label>Changed by</label>
										<name>Laura Mandell</name>
										<list>
									...
									<change who="#LM" when="2012-01-15" n="5">
										<label>Changed by</label>
										<name>Laura Mandell</name>
										<list>
									...
									<change n="4" when="2011-12-02" who="#DR">
										<label>Changed by</label>
									...
									<change n="3" when="2010-08-15" who="#AB">
										<label>Changed by</label>
									...
									<change n="2" when="2010-07-22" who="#LM">
										<label>Changed by</label>
									...
									<change n="1" when="2010-06-27" who="#AB">
										<label>Changed by</label>
		...
		<xsl:variable name="letterNumber">
			<xsl:value-of select="tei:text/tei:body/tei:div[@type='letter']/@n"/>
		</xsl:variable>
		<xsl:variable name="code">
			<xsl:value-of select="concat('letterEEd.26.', $letterNumber, '.xml')"/>
		</xsl:variable>
		<letter code="{$code}"/>
		<xsl:text>
				</xsl:text>
