...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	08-created for single items in LBs 8/2/2025-->
	...
	<!-- =======================================================
		...
		8.  drama
		9. letters
		10. notes and back matter
		11.  page numbers and forme work
	...
	<xsl:param name="facsType">.jpg</xsl:param>
	<xsl:param name="baseURLpgFacs">../pgImgsLB1800Duke/</xsl:param>
	...
	<xsl:template match="list">
		<xsl:for-each select="item">
			<xsl:apply-templates select="document(@code)/tei:TEI">
				<xsl:with-param name="edID" select="parent::list/@id" tunnel="yes"/>
			</xsl:apply-templates>
		</xsl:for-each>
	</xsl:template>
	...
	<xsl:template match="tei:add"> <!-- added a bunch here -->
		...
		<span>
			...
			<xsl:apply-templates/>
			<xsl:if test="@hand">
				<sup><xsl:value-of select="@hand"/></sup>
			</xsl:if>
		</span>
	</xsl:template>
	...
	<xsl:template match="tei:ref">
		<xsl:variable name="url">
			<xsl:choose>
				<xsl:when test="contains(@target, 'RIMEfstc')">
					<xsl:text>RIMEstc</xsl:text>
				</xsl:when>
				<xsl:when test="ends-with(@target, 'f')">
					<xsl:value-of select="substring-before(@target, 'f')"/>
				</xsl:when>
				<xsl:otherwise>
		...
		<xsl:choose>
			<xsl:when test="@type = 'endnote'"><!--in the splurge out, I put the whole html plus #-->
				<span class="sup">
					<a href="{@target}">
						<xsl:apply-templates/>
					</a>
			...
			<!-- for processing the single-poem tocs; I removed the # in splurge out. -->
